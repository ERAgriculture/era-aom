#!/usr/bin/env python3
"""Build governed feed-material form, role, and external-label review cohort."""
import csv
import json
...
for material in inventory:
    ...
    existing_forms = sorted({
        ...
        if row["target_property"] == "aom:physicalForm"
    })
    ...
    if existing_forms:
        ...
        form_rationale = "Existing reviewed physical-form assertion remains authoritative."
    elif has_grinding:
        form_disposition = "approved_comminuted_form"
        form_target = "Comminuted solid form"
        form_rationale = (
            "Approved Grinding process supports broad comminuted-solid form; "
            "particle size and meal/powder subtype remain unspecified."
        )
        assertions.append({
            "feed_material_id": concept_id,
            "target_property": "aom:physicalForm",
            "target_concept_id": COMMINUTED_FORM_ID,
            "target_label": "Comminuted solid form",
            "rule_id": "STRUCT-FORM-GRINDING",
            "status": "approved-generated",
            "reviewer": REVIEWER,
            "review_date": DATE,
            "evidence": "data/livestock-staging/approved_generated_feed_material_facets.csv;docs/decisions/0041-feed-material-structural-model.md",
        ...
        form_rationale = (
            ...
            "or processed form; no physical-form assertion promoted."
        )
    else:
        form_disposition = "not_in_form_cohort"
        form_target = ""
        form_rationale = "No reviewed grinding result or governed physical-form assertion."
    ...
    review_rows.append({
        ...
        "evidence": "docs/decisions/0041-feed-material-structural-model.md",
        "rationale": form_rationale + " " + role_rationale,
    })
