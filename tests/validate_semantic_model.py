...
assert len(material_facets) == 1598
...
assert len(facets) == 9
assert len({row["facet_id"] for row in facets}) == 9
...
final_sources = {row["source_name"] for row in taxon_final}
approved_final = {row["source_value"] for row in taxon_value_bindings} & final_sources
assert approved_final == final_sources
final_holds = {
    row["source_value"] for row in taxon_value_bindings
...
for value_class in {
    ...
    "ProductRole", "IngredientConstituent", "FeedMaterialComponent",
}:
...
whole_crop_maize_silage = URIRef("urn:era-aom:livestock:AOM_001326")
assert (whole_crop_maize_silage, RDF.type, URIRef("urn:era-aom:schema:FeedMaterial")) in binding_graph
...
deprecated_maize_whole_ensiled = URIRef("urn:era-aom:livestock:AOM_006072")
assert not any(binding_graph.predicate_objects(deprecated_maize_whole_ensiled))
...
print("Semantic model validation passed: 50 dispositions; 13 structural, 418 value bindings, 9 facets, 1598 material assertions")
