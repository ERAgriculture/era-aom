...
assert len(approved) == 1917
assert len({row["concept_id"] for row in approved}) == 1917
assert Counter(row["definition_method"] for row in approved) == {
    "composed_from_approved_semantic_facets": 980,
...
assert sum(row["source_column"] == "approved_definition_enrichment" for row in definitions) == 1917
defined = {row["concept_id"] for row in definitions}
active = [row for row in concepts if row["status"] != "deprecated"]
assert sum(row["concept_id"] not in defined for row in active) == 210
...
assert manifest["counts"]["approved_definition_enrichments"] == 1917
print("Definition enrichment validation passed: 1,917 definitions; 210 active gaps remain")
