...
assert len(approved) == 2184
assert len({row["concept_id"] for row in approved}) == 2184
assert Counter(row["definition_method"] for row in approved) == {
    "composed_from_approved_semantic_facets": 1106,
    "composed_from_bounded_workbook_material_scope": 62,
    "promoted_reviewed_scope_note": 206,
    ...
    "composed_from_canonical_workbook_identity_scope": 105,
    ...
    "feed_taxonomy_axis_definition_replacement": 39,
    "feed_product_kind_definition_replacement": 13,
    "process_axis_definition_replacement": 46,
    "component_chemical_identity_definition_replacement": 34,
    "composition_form_definition_replacement": 22,
}
...
assert sum(row["source_column"] == "approved_definition_enrichment" for row in definitions) == 2184
...
assert "source identity “maize”" in definition and "Whole-grain component retention" in definition
manifest = json.loads((ROOT / "dist/livestock-staging/manifest.json").read_text())
assert manifest["counts"]["approved_definition_enrichments"] == 2184
print("Definition enrichment validation passed: 2,184 definitions; 15 active integrity holds remain")
