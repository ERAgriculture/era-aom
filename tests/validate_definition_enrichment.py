...
assert len(approved) == 1744
assert len({row["concept_id"] for row in approved}) == 1744
...
assert sum(row["source_column"] == "approved_definition_enrichment" for row in definitions) == 1744
defined = {row["concept_id"] for row in definitions}
active = [row for row in concepts if row["status"] != "deprecated"]
assert sum(row["concept_id"] not in defined for row in active) == 383
...
assert manifest["counts"]["approved_definition_enrichments"] == 1744
print("Definition enrichment validation passed: 1,744 definitions; 383 active gaps remain")
