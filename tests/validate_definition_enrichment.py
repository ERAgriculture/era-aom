...
assert len(approved) == 2112
assert len({row["concept_id"] for row in approved}) == 2112
assert Counter(row["definition_method"] for row in approved) == {
    ...
    "promoted_reviewed_scope_note": 266,
...
assert sum(row["source_column"] == "approved_definition_enrichment" for row in definitions) == 2112
...
assert manifest["counts"]["approved_definition_enrichments"] == 2112
print("Definition enrichment validation passed: 2,112 definitions; 12 active integrity holds remain")
