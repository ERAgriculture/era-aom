...
assert len(rows) == 781 and len({row["concept_id"] for row in rows}) == 781
assert Counter(row["domain"] for row in rows) == {
    "feed_material": 536, "outcome": 80, "rearing_stage": 67,
    "taxon": 45, "management": 35, "farming_system": 16, "core_root": 2,
}
assert summary == {
    "prior_active_gaps": 781,
    "approved_structural_definitions": 243,
    "research_required": 536,
    "expert_review_required": 2,
    "remaining_after_approval": 538,
    "routes": {
...
print("Definition-gap classification validation passed: 781 routed; 538 remain")
