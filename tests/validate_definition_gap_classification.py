...
assert len(rows) == 369 and len({row["concept_id"] for row in rows}) == 369
assert Counter(row["domain"] for row in rows) == {
    "feed_material": 126, "outcome": 80, "rearing_stage": 67,
    "taxon": 45, "management": 35, "farming_system": 16,
}
assert summary == {
    "prior_active_gaps": 369,
    "approved_structural_definitions": 243,
    "research_required": 126,
    "expert_review_required": 0,
    "remaining_after_approval": 126,
    "routes": {
        "approved_structural_definition": 243,
        "research_related_mapping_insufficient": 88,
        "research_source_workbook": 21,
...
print("Definition-gap classification validation passed: 369 routed; 126 remain")
