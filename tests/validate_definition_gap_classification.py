...
assert len(rows) == 373 and len({row["concept_id"] for row in rows}) == 373
assert Counter(row["domain"] for row in rows) == {
    "feed_material": 130, "outcome": 80, "rearing_stage": 67,
    "taxon": 45, "management": 35, "farming_system": 16,
}
assert summary == {
    "prior_active_gaps": 373,
    "approved_structural_definitions": 243,
    "research_required": 130,
    "expert_review_required": 0,
    "remaining_after_approval": 130,
    "routes": {
...
print("Definition-gap classification validation passed: 373 routed; 130 remain")
