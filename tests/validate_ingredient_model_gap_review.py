...
assert summary == {
    "remaining_exceptions": 25, "model_gap_families": 5,
...
assert sum(int(row["concept_count"]) for row in gaps) == 25
...
assert len({item for row in gaps for item in row["concept_ids"].split(";")}) == 25
...
print("Ingredient model-gap review validation passed: 25 exceptions -> 5 model families; 7 clusters")
