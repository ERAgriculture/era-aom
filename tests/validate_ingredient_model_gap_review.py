...
assert summary == {
    "remaining_exceptions": 29, "model_gap_families": 6,
    "remaining_signature_clusters": 6, "governed_label_overrides": 8,
    "high_confidence_deprecation_reviews": 1, "automatic_identity_changes": 0,
}
assert sum(int(row["concept_count"]) for row in gaps) == 29
assert {row["gap_id"] for row in gaps} == {
    "standalone_material_identity", "whole_grain_integrity",
...
assert len({item for row in gaps for item in row["concept_ids"].split(";")}) == 29
...
assert all(
    ...
    for key, row in by_cluster.items() if key in {"INGCLUSTER-0003", "INGCLUSTER-0004", "INGCLUSTER-0005", "INGCLUSTER-0006"}
)
...
print("Ingredient model-gap review validation passed: 29 exceptions -> 6 model families; 6 clusters")
