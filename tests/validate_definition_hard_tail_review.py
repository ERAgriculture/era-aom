...
assert Counter(row["status"] for row in rows) == {"held": 69, "approved": 141}
assert Counter(row["decision"] for row in rows) == {
    "hold_expert_evidence_required": 69,
    ...
    "approve_bounded_workbook_material_scope": 40,
}
assert len(facets) == 147
assert Counter(row["target_property"] for row in facets) == {
    "aom:productRole": 56, "aom:ingredientPart": 43,
    "aom:ingredientConstituent": 17, "aom:physicalForm": 18,
    "aom:processingMethod": 13,
}
...
print("Definition hard-tail review passed: 141 approved; 69 actionable holds; 147 facets")
